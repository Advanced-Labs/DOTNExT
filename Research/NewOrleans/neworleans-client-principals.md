# NewOrleans Client Principals
## Architecture Specification for DOTNExT Fork

**Version**: 1.0  
**Status**: Architecture Design  
**Context**: This document specifies how NewOrleans elevates external clients to first-class participants in the actor system, with identity, state, and presence management.  
**Audience**: AI assistants and developers working on DOTNExT  
**Importance**: This is foundational architecture for NewOrleans 1.0 — not a future enhancement.
...
6. [Integration with NewOrleans Events](#6-integration-with-neworleans-events)
...
**NewOrleans Client Principals** transforms external clients from anonymous connection endpoints into first-class participants in the distributed actor system.
...
**Why this matters**: This architecture makes NewOrleans a complete platform for distributed applications, not just a grain framework. Clients aren't second-class citizens reaching into the system — they're participants with their own actor identity.
...
// NewOrleans Client Principals: subscription tied to session
...
## 6. Integration with NewOrleans Events
...
```csharp
// NewOrleans can automatically route subscriptions through your session
var player = client.GetGrain<IPlayerGrain>("player-1");
...
siloBuilder.ConfigureNewOrleans(options =>
{
...
public static class CallContext
{
    ...
    {
        return RequestContext.Get("NewOrleans.CallingAccount") as IAccountGrain;
    }
    ...
    {
        return RequestContext.Get("NewOrleans.CallingSession") as ISessionGrain;
    }
    ...
    {
        return RequestContext.Get("NewOrleans.CallingConnection") as IConnectionGrain;
    }
}
```
...
│  - RequestContext["NewOrleans.CallingAccount"] = accountGrain               │
│  - RequestContext["NewOrleans.CallingSession"] = sessionGrain               │
│  - RequestContext["NewOrleans.CallingConnection"] = connectionGrain         │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
public class ClientPrincipalCallFilter : IOutgoingGrainCallFilter
{
    public async Task Invoke(IOutgoingGrainCallContext context)
    {
        ...
        RequestContext.Set("NewOrleans.CallingAccount", _accountGrain);
        RequestContext.Set("NewOrleans.CallingSession", _sessionGrain);
        RequestContext.Set("NewOrleans.CallingConnection", _connectionGrain);
...
[StorageProvider(ProviderName = "NewOrleans.Accounts")]
...
[StorageProvider(ProviderName = "NewOrleans.Sessions")]
...
```csharp
// NewOrleans client with principal support
var client = new NewOrleansClientBuilder()
...
var client = new NewOrleansClientBuilder().Build();
await client.ConnectAsync();
...
| Feature | Plain Orleans | NewOrleans with Client Principals |
|---------|---------------|-----------------------------------|
...
- [ ] Integrate with NewOrleans Events
...
public class NewOrleansOptions
{
...
public class PrincipalOptions
{
    ...
    public string AccountStorageProvider { get; set; } = "NewOrleans.Accounts";
    public string SessionStorageProvider { get; set; } = "NewOrleans.Sessions";
}
...
public static class CallContext
{
    ...
    /// </summary>
    public static IAccountGrain? GetCallingAccount()
        => RequestContext.Get("NewOrleans.CallingAccount") as IAccountGrain;
    ...
    /// </summary>
    public static ISessionGrain? GetCallingSession()
        => RequestContext.Get("NewOrleans.CallingSession") as ISessionGrain;
    ...
    /// </summary>
    public static IConnectionGrain? GetCallingConnection()
        => RequestContext.Get("NewOrleans.CallingConnection") as IConnectionGrain;
