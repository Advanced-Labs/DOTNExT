...
public class HelloGrain : Grain, IHelloGrain
{
    ...
    public override Task OnActivateAsync(CancellationToken cancellationToken)
    {
        _logger.LogInformation(
            "HelloGrain {GrainId} activated (Loaded Dynamically!)",
    ...
    {
        _callCount++;
        var message = $"Hello, {name}! You are visitor #{_callCount}. (Loaded dynamically at runtime!)";
...
public class CounterGrain : Grain, ICounterGrain
{
    ...
    {
        _logger.LogInformation(
            "CounterGrain {GrainId} activated with count: {Count}",
...
public class EchoGrain : Grain, IEchoGrain
{
    ...
    {
        _logger.LogInformation(
            "Echoing complex data: Name={Name}, Value={Value}, TagCount={TagCount}",
            data.Name,
            data.Value,
            data.Tags.Count);
        ...
        {
            Name = $"Echo of {data.Name}",
            Value = data.Value * 2,
            Timestamp = DateTime.UtcNow,
            Tags = new List<string>(data.Tags) { "echoed" }
        };
