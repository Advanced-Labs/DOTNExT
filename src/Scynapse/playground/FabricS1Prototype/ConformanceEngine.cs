...
internal sealed class ConformanceEngine
{
    ...
    private const string SliceProfileM1S12 = "M1-S12";
    ...
    private static readonly HashSet<string> KnownReferenceGrantProofFreshnessStatuses = new(StringComparer.Ordinal)
    {
        "fresh",
        "stale"
    };

    private static readonly HashSet<string> KnownReferenceGrantProofReplayStatuses = new(StringComparer.Ordinal)
    {
        "clear",
        "replayed"
    };
    ...
    private static void ValidateMessageFieldRules(FixtureCase fixture, string sliceProfile, List<ConformanceError> errors)
    {
        foreach (var message in fixture.Messages)
        {
            ...
            if (string.Equals(sliceProfile, SliceProfileM1S1, StringComparison.Ordinal) ||
                ...
                string.Equals(sliceProfile, SliceProfileM1S12, StringComparison.Ordinal))
            {
            ...
            if (string.Equals(sliceProfile, SliceProfileM1S12, StringComparison.Ordinal))
            {
                ValidateM1S12HandshakeInitIssuerFields(fixture, message, errors);
                ValidateM1S12ReferenceGrantIssuerFields(fixture, message, errors);
            }
    ...
    private static void ValidateM1S9ReferenceGrantFreshnessFields(FixtureCase fixture, FixtureMessage message, List<ConformanceError> errors)
    {
        if (!string.Equals(message.Type, "HandshakeAccept", StringComparison.Ordinal))
        {
            return;
        ...
        var hasFreshnessStatus = message.Body.Value.TryGetProperty("reference_grant_proof_freshness_status", out var freshnessStatusElement);
        var hasReplayStatus = message.Body.Value.TryGetProperty("reference_grant_proof_replay_status", out var replayStatusElement);

        var referenceGrantStatus = GetBodyString(message, "reference_grant_status");
        if (!string.Equals(referenceGrantStatus, "active", StringComparison.Ordinal))
        {
            if (hasFreshnessStatus || hasReplayStatus)
            {
                AddError(errors, "L2", "E3144_M1S9_REFERENCE_GRANT_PROOF_FRESHNESS_FIELDS_FORBIDDEN", $"{fixture.Id}/{message.Type} freshness/replay proof fields are forbidden when reference_grant_status is not 'active'.");
            }

            return;
        }

        if (!hasFreshnessStatus)
        {
            AddError(errors, "L2", "E3140_M1S9_REFERENCE_GRANT_PROOF_FRESHNESS_STATUS_REQUIRED", $"{fixture.Id}/{message.Type} requires 'reference_grant_proof_freshness_status' when reference_grant_status='active'.");
        }
        else if (freshnessStatusElement.ValueKind != JsonValueKind.String)
        {
            AddError(errors, "L2", "E3141_M1S9_REFERENCE_GRANT_PROOF_FRESHNESS_STATUS_INVALID", $"{fixture.Id}/{message.Type} field 'reference_grant_proof_freshness_status' must be a string.");
        }
        else
        {
            var freshnessStatus = freshnessStatusElement.GetString() ?? string.Empty;
            if (!KnownReferenceGrantProofFreshnessStatuses.Contains(freshnessStatus))
            {
                AddError(errors, "L2", "E3141_M1S9_REFERENCE_GRANT_PROOF_FRESHNESS_STATUS_INVALID", $"{fixture.Id}/{message.Type} field 'reference_grant_proof_freshness_status' value '{freshnessStatus}' is invalid.");
            }
        }

        if (!hasReplayStatus)
        {
            AddError(errors, "L2", "E3142_M1S9_REFERENCE_GRANT_PROOF_REPLAY_STATUS_REQUIRED", $"{fixture.Id}/{message.Type} requires 'reference_grant_proof_replay_status' when reference_grant_status='active'.");
        }
        else if (replayStatusElement.ValueKind != JsonValueKind.String)
        {
            AddError(errors, "L2", "E3143_M1S9_REFERENCE_GRANT_PROOF_REPLAY_STATUS_INVALID", $"{fixture.Id}/{message.Type} field 'reference_grant_proof_replay_status' must be a string.");
        }
        else
        {
            var replayStatus = replayStatusElement.GetString() ?? string.Empty;
            if (!KnownReferenceGrantProofReplayStatuses.Contains(replayStatus))
            {
                AddError(errors, "L2", "E3143_M1S9_REFERENCE_GRANT_PROOF_REPLAY_STATUS_INVALID", $"{fixture.Id}/{message.Type} field 'reference_grant_proof_replay_status' value '{replayStatus}' is invalid.");
            }
        }
    }

    private static void ValidateM1S10HandshakeInitFields(FixtureCase fixture, FixtureMessage message, List<ConformanceError> errors)
    {
        if (!string.Equals(message.Type, "HandshakeInit", StringComparison.Ordinal))
        {
            return;
        }
        ...
        if (!message.Body.Value.TryGetProperty("requester_subject_ref", out var requesterSubjectElement))
        {
            AddError(errors, "L2", "E3160_M1S10_REQUESTER_SUBJECT_REF_REQUIRED", $"{fixture.Id}/{message.Type} requires typed 'requester_subject_ref' for claim binding.");
        }
        else if (requesterSubjectElement.ValueKind != JsonValueKind.String || !IsTypedIdentifier(requesterSubjectElement.GetString() ?? string.Empty))
        {
            AddError(errors, "L2", "E3161_M1S10_REQUESTER_SUBJECT_REF_INVALID", $"{fixture.Id}/{message.Type} field 'requester_subject_ref' must be a typed identifier.");
        }

        var requestedScope = GetBodyString(message, "requested_scope");
        if (string.IsNullOrWhiteSpace(requestedScope))
        {
            AddError(errors, "L2", "E3162_M1S10_REQUESTED_SCOPE_INVALID", $"{fixture.Id}/{message.Type} requires non-empty string 'requested_scope' for claim binding.");
        }

        var requestedOps = GetBodyStringArray(message, "requested_ops");
        if (requestedOps.Count == 0)
        {
            AddError(errors, "L2", "E3163_M1S10_REQUESTED_OPS_INVALID", $"{fixture.Id}/{message.Type} requires non-empty string array 'requested_ops' for claim binding.");
        }
    }

    private static void ValidateM1S10ReferenceGrantClaimFields(FixtureCase fixture, FixtureMessage message, List<ConformanceError> errors)
    {
        if (!string.Equals(message.Type, "HandshakeAccept", StringComparison.Ordinal))
        {
            return;
        ...
        var hasClaimSubject = message.Body.Value.TryGetProperty("reference_grant_claim_subject_ref", out var claimSubjectElement);
        var hasClaimScope = message.Body.Value.TryGetProperty("reference_grant_claim_scope", out var claimScopeElement);
        var hasClaimAction = message.Body.Value.TryGetProperty("reference_grant_claim_action", out var claimActionElement);

        var referenceGrantStatus = GetBodyString(message, "reference_grant_status");
        if (!string.Equals(referenceGrantStatus, "active", StringComparison.Ordinal))
        {
            if (hasClaimSubject || hasClaimScope || hasClaimAction)
            {
                AddError(errors, "L2", "E3174_M1S10_REFERENCE_GRANT_CLAIM_FIELDS_FORBIDDEN", $"{fixture.Id}/{message.Type} claim binding fields are forbidden when reference_grant_status is not 'active'.");
            }
        ...
        if (!hasClaimSubject)
        {
            AddError(errors, "L2", "E3164_M1S10_REFERENCE_GRANT_CLAIM_SUBJECT_REQUIRED", $"{fixture.Id}/{message.Type} requires typed 'reference_grant_claim_subject_ref' when reference_grant_status='active'.");
        }
        else if (claimSubjectElement.ValueKind != JsonValueKind.String || !IsTypedIdentifier(claimSubjectElement.GetString() ?? string.Empty))
        {
            AddError(errors, "L2", "E3165_M1S10_REFERENCE_GRANT_CLAIM_SUBJECT_INVALID", $"{fixture.Id}/{message.Type} field 'reference_grant_claim_subject_ref' must be a typed identifier.");
        }

        if (!hasClaimScope)
        {
            AddError(errors, "L2", "E3166_M1S10_REFERENCE_GRANT_CLAIM_SCOPE_REQUIRED", $"{fixture.Id}/{message.Type} requires non-empty string 'reference_grant_claim_scope' when reference_grant_status='active'.");
        }
        else if (claimScopeElement.ValueKind != JsonValueKind.String || string.IsNullOrWhiteSpace(claimScopeElement.GetString()))
        {
            AddError(errors, "L2", "E3167_M1S10_REFERENCE_GRANT_CLAIM_SCOPE_INVALID", $"{fixture.Id}/{message.Type} field 'reference_grant_claim_scope' must be a non-empty string.");
        }

        if (!hasClaimAction)
        {
            AddError(errors, "L2", "E3168_M1S10_REFERENCE_GRANT_CLAIM_ACTION_REQUIRED", $"{fixture.Id}/{message.Type} requires non-empty string 'reference_grant_claim_action' when reference_grant_status='active'.");
        }
        else if (claimActionElement.ValueKind != JsonValueKind.String || string.IsNullOrWhiteSpace(claimActionElement.GetString()))
        {
            AddError(errors, "L2", "E3169_M1S10_REFERENCE_GRANT_CLAIM_ACTION_INVALID", $"{fixture.Id}/{message.Type} field 'reference_grant_claim_action' must be a non-empty string.");
        }
    }
    ...
    private static void ValidateM1S12HandshakeInitIssuerFields(FixtureCase fixture, FixtureMessage message, List<ConformanceError> errors)
    {
        if (!string.Equals(message.Type, "HandshakeInit", StringComparison.Ordinal))
        {
            return;
        ...
        if (!message.Body.Value.TryGetProperty("requested_grant_issuer_ref", out var requestedIssuerElement))
        {
            AddError(errors, "L2", "E3200_M1S12_REQUESTED_GRANT_ISSUER_REF_REQUIRED", $"{fixture.Id}/{message.Type} requires typed 'requested_grant_issuer_ref' for issuer binding.");
            return;
        }

        if (requestedIssuerElement.ValueKind != JsonValueKind.String || !IsTypedIdentifier(requestedIssuerElement.GetString() ?? string.Empty))
        {
            AddError(errors, "L2", "E3201_M1S12_REQUESTED_GRANT_ISSUER_REF_INVALID", $"{fixture.Id}/{message.Type} field 'requested_grant_issuer_ref' must be a typed identifier.");
        }
    }

    private static void ValidateM1S12ReferenceGrantIssuerFields(FixtureCase fixture, FixtureMessage message, List<ConformanceError> errors)
    {
        if (!string.Equals(message.Type, "HandshakeAccept", StringComparison.Ordinal))
        {
            return;
        ...
        var hasIssuerClaim = message.Body.Value.TryGetProperty("reference_grant_claim_issuer_ref", out var issuerClaimElement);
        var referenceGrantStatus = GetBodyString(message, "reference_grant_status");
        if (!string.Equals(referenceGrantStatus, "active", StringComparison.Ordinal))
        {
            if (hasIssuerClaim)
            {
                AddError(errors, "L2", "E3204_M1S12_REFERENCE_GRANT_CLAIM_ISSUER_REF_FORBIDDEN", $"{fixture.Id}/{message.Type} issuer claim field is forbidden when reference_grant_status is not 'active'.");
            }
        ...
        if (!hasIssuerClaim)
        {
            AddError(errors, "L2", "E3202_M1S12_REFERENCE_GRANT_CLAIM_ISSUER_REF_REQUIRED", $"{fixture.Id}/{message.Type} requires typed 'reference_grant_claim_issuer_ref' when reference_grant_status='active'.");
            return;
        }

        if (issuerClaimElement.ValueKind != JsonValueKind.String || !IsTypedIdentifier(issuerClaimElement.GetString() ?? string.Empty))
        {
            AddError(errors, "L2", "E3203_M1S12_REFERENCE_GRANT_CLAIM_ISSUER_REF_INVALID", $"{fixture.Id}/{message.Type} field 'reference_grant_claim_issuer_ref' must be a typed identifier.");
        }
    }
        ...
        switch (message.Type)
        {
            ...
            case "HandshakeInit":
            {
                ...
                if (string.Equals(context.SliceProfile, SliceProfileM1S10, StringComparison.Ordinal) ||
                    ...
                    string.Equals(context.SliceProfile, SliceProfileM1S12, StringComparison.Ordinal))
                {
                    ...
                    context.RequestedOps = GetBodyStringArray(message, "requested_ops");
                }

                if (string.Equals(context.SliceProfile, SliceProfileM1S12, StringComparison.Ordinal))
                {
                    context.RequestedGrantIssuerRef = GetBodyString(message, "requested_grant_issuer_ref");
                }
            ...
            case "HandshakeAccept":
            {
                ...
                if (string.Equals(context.SliceProfile, SliceProfileM1S9, StringComparison.Ordinal))
                {
                    if (!TryEvaluateM1S5RelationTokenIntegrity(fixtureId, message, context, errors))
                    {
                        break;
                ...
                if (string.Equals(context.SliceProfile, SliceProfileM1S12, StringComparison.Ordinal))
                {
                    ...
                    if (!TryEvaluateM1S12ReferenceGrantIssuerBinding(fixtureId, message, context, errors))
                    {
                        break;
                    }
            ...
            case "RouteData":
            {
                ...
                {
                    RejectWithDeterministicDeny(fixtureId, context, errors, "E3062_M1S2_ROUTE_DATA_OUTSIDE_PROFILE", "TrustInsufficient", $"{fixtureId}/{message.Type} is only supported in M1-S2/M1-S3/M1-S4/M1-S5/M1-S6/M1-S7/M1-S8/M1-S9/M1-S10 runtime-bridge profiles.");
                    break;
                }
    ...
    private static bool TryEvaluateM1S9ReferenceGrantFreshness(string fixtureId, FixtureMessage message, OperationContext context, List<ConformanceError> errors)
    {
        var tokenTransport = GetBodyString(message, "token_transport");
        if (!string.Equals(tokenTransport, "reference", StringComparison.Ordinal))
        {
        ...
        var freshnessStatus = GetBodyString(message, "reference_grant_proof_freshness_status");
        if (string.Equals(freshnessStatus, "stale", StringComparison.Ordinal))
        {
            RejectWithDeterministicDeny(
                fixtureId,
                context,
                ...
                "GrantExpired",
                $"{fixtureId}/{message.Type} grant proof freshness is stale.");
            return false;
        }

        var replayStatus = GetBodyString(message, "reference_grant_proof_replay_status");
        if (string.Equals(replayStatus, "replayed", StringComparison.Ordinal))
        {
            RejectWithDeterministicDeny(
                fixtureId,
                context,
                ...
                "TrustInsufficient",
                $"{fixtureId}/{message.Type} grant proof replay was detected.");
            return false;
        }
    ...
    private static bool TryEvaluateM1S10ReferenceGrantClaimBinding(string fixtureId, FixtureMessage message, OperationContext context, List<ConformanceError> errors)
    {
        var tokenTransport = GetBodyString(message, "token_transport");
        if (!string.Equals(tokenTransport, "reference", StringComparison.Ordinal))
        {
        ...
        var claimSubject = GetBodyString(message, "reference_grant_claim_subject_ref");
        if (string.IsNullOrWhiteSpace(claimSubject) || !IsTypedIdentifier(claimSubject))
        {
            return true;
        }

        var claimScope = GetBodyString(message, "reference_grant_claim_scope");
        if (string.IsNullOrWhiteSpace(claimScope))
        {
            return true;
        }

        var claimAction = GetBodyString(message, "reference_grant_claim_action");
        if (string.IsNullOrWhiteSpace(claimAction))
        {
            return true;
        }

        if (string.IsNullOrWhiteSpace(context.RequesterSubjectRef) || !IsTypedIdentifier(context.RequesterSubjectRef))
        {
            return true;
        }

        if (!string.IsNullOrWhiteSpace(context.RequesterSubjectRef) &&
            !string.Equals(claimSubject, context.RequesterSubjectRef, StringComparison.Ordinal))
        {
            RejectWithDeterministicDeny(
                fixtureId,
                context,
                ...
                "TrustInsufficient",
                $"{fixtureId}/{message.Type} grant claim subject does not match requester subject.");
            return false;
        }

        if (!string.IsNullOrWhiteSpace(context.RequestedScope) &&
            !string.Equals(claimScope, context.RequestedScope, StringComparison.Ordinal))
        {
            RejectWithDeterministicDeny(
                fixtureId,
                context,
                ...
                "PolicyDenied",
                $"{fixtureId}/{message.Type} grant claim scope does not match requested scope.");
            return false;
        }

        if (context.RequestedOps.Count > 0 &&
            !context.RequestedOps.Contains(claimAction ?? string.Empty, StringComparer.Ordinal))
        {
            RejectWithDeterministicDeny(
                fixtureId,
                context,
                ...
                "PolicyDenied",
                $"{fixtureId}/{message.Type} grant claim action does not match requested operations.");
            return false;
        }
    ...
    private static bool TryEvaluateM1S12ReferenceGrantIssuerBinding(string fixtureId, FixtureMessage message, OperationContext context, List<ConformanceError> errors)
    {
        var tokenTransport = GetBodyString(message, "token_transport");
        if (!string.Equals(tokenTransport, "reference", StringComparison.Ordinal))
        {
        ...
        var issuerClaim = GetBodyString(message, "reference_grant_claim_issuer_ref");
        if (string.IsNullOrWhiteSpace(issuerClaim) || !IsTypedIdentifier(issuerClaim))
        {
            return true;
        }

        if (string.IsNullOrWhiteSpace(context.RequestedGrantIssuerRef) || !IsTypedIdentifier(context.RequestedGrantIssuerRef))
        {
            return true;
        }

        if (!string.Equals(issuerClaim, context.RequestedGrantIssuerRef, StringComparison.Ordinal))
        {
            RejectWithDeterministicDeny(
                fixtureId,
                context,
                ...
                "TrustInsufficient",
                $"{fixtureId}/{message.Type} grant issuer does not match requested grant issuer.");
            return false;
        }
    ...
    private static List<string> GetBodyStringArray(FixtureMessage message, string propertyName)
    {
        if (message.Body is null || message.Body.Value.ValueKind != JsonValueKind.Object)
        {
...
internal sealed class OperationContext
{
    ...
    public string? RequestedGrantIssuerRef { get; set; }
