...
namespace Scynapse.Streaming.NATS;
...
internal sealed class NatsConnectionManager
{
    ...
    public NatsConnectionManager(string providerName, ILoggerFactory loggerFactory, NatsOptions options)
    {
        ...
        if (this._options.NatsClientOptions is null)
        {
            ...
            {
                Name = $"Scynapse-{this._providerName}",
                SerializerRegistry =
        ...
        {
            var producerOptions = this._natsClientOptions with { Name = $"Scynapse-{this._providerName}-Producer-{i}" };
        ...
        try
        {
            ...
            if (!this._natsConnection.ServerInfo!.JetStreamAvailable)
            {
                this._logger.LogError(
                    "Unable to use {NatsServer} for Scynapse Stream Provider {ProviderName}: NATS JetStream is not available",
