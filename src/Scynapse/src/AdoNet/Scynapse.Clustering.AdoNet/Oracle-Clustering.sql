...
(
    ...
    CONSTRAINT "SCYNAPSEMEMBERSHIPVERSIONTA_PK" PRIMARY KEY ("DEPLOYMENTID")
);
/
...
(
    ...
    CONSTRAINT "SCYNAPSEMEMBERSHIPTABLE_PK" PRIMARY KEY ("DEPLOYMENTID", "ADDRESS", "PORT", "GENERATION"),
    CONSTRAINT "SCYNAPSEMEMBERSHIPTABLE_FK1" FOREIGN KEY ("DEPLOYMENTID")
	  REFERENCES "SCYNAPSEMEMBERSHIPVERSIONTABLE" ("DEPLOYMENTID") ENABLE
);
/
...
CREATE OR REPLACE FUNCTION InsertMembership(PARAM_DEPLOYMENTID IN NVARCHAR2, PARAM_IAMALIVETIME IN TIMESTAMP, PARAM_SILONAME IN NVARCHAR2, PARAM_HOSTNAME IN NVARCHAR2, PARAM_ADDRESS IN VARCHAR2,
  ...
  BEGIN
    ...
    (
      SELECT 1 FROM ScynapseMembershipTable WHERE
...
BEGIN
  INSERT INTO ScynapseMembershipVersionTable
      ...
      (
        SELECT 1 FROM ScynapseMembershipVersionTable WHERE
        DeploymentId = PARAM_DEPLOYMENTID AND PARAM_DEPLOYMENTID IS NOT NULL
      );
      rowCount := SQL%ROWCOUNT;
