...
using Scynapse.Persistence.AdoNet.Storage;
...
namespace Scynapse.Configuration
{
    ...
    public class AdoNetGrainStorageOptions : IStorageProviderSerializerOptions
    {
        ...
        /// <summary>
        /// Sets legacy Scynapse v3-compatible hash picker to use for this storage provider. Invoke this method if you need to run
        /// Scynapse v7+ silo against existing Scynapse v3-initialized database and keep existing grain state.
        /// </summary>
        public void UseScynapse3CompatibleHasher()
        {
            // content-aware hashing with different pickers, unable to use standard StorageHasherPicker
            this.HashPicker = new Scynapse3CompatibleStorageHashPicker();
        }
    }
    ...
    public class AdoNetGrainStorageOptionsValidator : IConfigurationValidator
    {
        ...
        {
            this.options = configurationOptions ?? throw new ScynapseConfigurationException($"Invalid AdoNetGrainStorageOptions for AdoNetGrainStorage {name}. Options is required.");
            this.name = name;
        }
        ...
        public void ValidateConfiguration()
        {
            ...
            {
                throw new ScynapseConfigurationException($"Invalid {nameof(AdoNetGrainStorageOptions)} values for {nameof(AdoNetGrainStorage)} \"{name}\". {nameof(options.Invariant)} is required.");
            }

            if (string.IsNullOrWhiteSpace(this.options.ConnectionString))
            {
                throw new ScynapseConfigurationException($"Invalid {nameof(AdoNetGrainStorageOptions)} values for {nameof(AdoNetGrainStorage)} \"{name}\". {nameof(options.ConnectionString)} is required.");
            }

            if (this.options.HashPicker == null)
            {
                throw new ScynapseConfigurationException($"Invalid {nameof(AdoNetGrainStorageOptions)} values for {nameof(AdoNetGrainStorage)} {name}. {nameof(options.HashPicker)} is required.");
            }
        }
    }
    ...
    public class DefaultAdoNetGrainStorageOptionsHashPickerConfigurator : IPostConfigureOptions<AdoNetGrainStorageOptions>
    {
        public void PostConfigure(string name, AdoNetGrainStorageOptions options)
        {
            ...
            options.HashPicker = new StorageHasherPicker(new[] { new ScynapseDefaultHasher() });
        }
    }
}
