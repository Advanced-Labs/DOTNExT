...
-- is recognized in the storage by the grain type and its ID, which are unique in Scynapse silo.
...
IF OBJECT_ID(N'[ScynapseStorage]', 'U') IS NULL
CREATE TABLE ScynapseStorage
(
    -- These are for the book keeping. Scynapse calculates
...
IF NOT EXISTS(SELECT * FROM sys.indexes WHERE name = 'IX_ScynapseStorage' AND object_id = OBJECT_ID('ScynapseStorage'))
BEGIN
	CREATE NONCLUSTERED INDEX IX_ScynapseStorage ON ScynapseStorage(GrainIdHash, GrainTypeHash);
END
...
ALTER TABLE ScynapseStorage SET(LOCK_ESCALATION = DISABLE);

-- A feature with ID is compression. If it is supported, it is used for ScynapseStorage table. This is an Enterprise feature.
...
BEGIN
    ALTER TABLE ScynapseStorage REBUILD PARTITION = ALL WITH(DATA_COMPRESSION = PAGE);
END

INSERT INTO ScynapseQuery(QueryKey, QueryText)
SELECT
    'WriteToStorageKey',
    '-- When Scynapse is running in normal, non-split state, there will
    -- be only one grain with the given ID and type combination only. This
    -- grain saves states mostly serially if Scynapse guarantees are upheld. Even
    ...
    --
    -- If the version number explicitly returned is still the same, Scynapse interprets it so the update did not succeed
    -- and throws an InconsistentStateException.
    --
    -- See further information at https://learn.microsoft.com/dotnet/scynapse/grains/grain-persistence.
    BEGIN TRANSACTION;
    ...
    -- The NULL value is supplied by Scynapse when the state is new.
    IF @GrainStateVersion IS NOT NULL
    BEGIN
        UPDATE ScynapseStorage
        SET
    ...
    BEGIN
        ...
        SELECT
         ...
         (
            ...
            SELECT 1
            FROM ScynapseStorage WITH(XLOCK, ROWLOCK, HOLDLOCK, INDEX(IX_ScynapseStorage))
            WHERE
                GrainIdHash = @GrainIdHash AND @GrainIdHash IS NOT NULL
                AND GrainTypeHash = @GrainTypeHash AND @GrainTypeHash IS NOT NULL
                AND (GrainIdN0 = @GrainIdN0 OR @GrainIdN0 IS NULL)
