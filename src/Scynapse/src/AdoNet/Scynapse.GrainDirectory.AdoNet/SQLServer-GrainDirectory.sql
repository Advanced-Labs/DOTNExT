/*
Scynapse Grain Directory.
This table stores the location of all grains in the cluster.

NOTE:
The combination of ClusterId, ProviderId, and GrainId forms the primary key for the ScynapseGrainDirectory table.
...
*/
CREATE TABLE ScynapseGrainDirectory
(
    ...
    CONSTRAINT PK_ScynapseGrainDirectory PRIMARY KEY CLUSTERED
    (
        ClusterId ASC,
        ProviderId ASC,
        GrainId ASC
...
(
    SELECT
        1
    FROM
        ScynapseGrainDirectory WITH (UPDLOCK, HOLDLOCK)
    WHERE
        ClusterId = @ClusterId
...
BEGIN
    ...
    INSERT INTO ScynapseGrainDirectory
    (
        ClusterId,
        ProviderId,
        GrainId,
...
DELETE ScynapseGrainDirectory
FROM ScynapseGrainDirectory
WHERE
    ClusterId = @ClusterId
    AND ProviderId = @ProviderId
    AND GrainId = @GrainId
...
DELETE ScynapseGrainDirectory
FROM ScynapseGrainDirectory WITH (TABLOCKX)
WHERE
    ClusterId = @ClusterId
    AND ProviderId = @ProviderId
