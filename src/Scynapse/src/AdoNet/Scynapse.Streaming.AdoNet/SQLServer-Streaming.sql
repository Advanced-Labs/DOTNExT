/*
Scynapse Stream Message Sequence.
...
*/
CREATE SEQUENCE ScynapseStreamMessageSequence
AS BIGINT
START WITH 1
INCREMENT BY 1
NO MAXVALUE
...
/*
Scynapse Streaming Message Queue.

This table stores queued messages awaiting processing by Scynapse.
...
*/
CREATE TABLE ScynapseStreamMessage
(
	...
	CONSTRAINT PK_ScynapseStreamMessage PRIMARY KEY CLUSTERED
	(
		ServiceId ASC,
        ProviderId ASC,
		QueueId ASC,
...
(
	...
	CONSTRAINT PK_ScynapseStreamDeadLetter PRIMARY KEY CLUSTERED
	(
		ServiceId ASC,
        ProviderId ASC,
		QueueId ASC,
...
(
	...
	CONSTRAINT PK_ScynapseStreamControl PRIMARY KEY CLUSTERED
	(
		ServiceId ASC,
        ProviderId ASC,
		QueueId ASC
...
/* Queues a message to the Scynapse Streaming Message Queue */
...
DECLARE @MessageId BIGINT = NEXT VALUE FOR ScynapseStreamMessageSequence;
DECLARE @Now DATETIME2(7) = SYSUTCDATETIME();
DECLARE @ExpiresOn DATETIME2(7) = DATEADD(SECOND, @ExpiryTimeout, @Now);

INSERT INTO ScynapseStreamMessage
(
	ServiceId,
    ProviderId,
	QueueId,
...
/* Gets message batches from the Scynapse Streaming Message Queue */
...
BEGIN
    ...
    MERGE ScynapseStreamControl WITH (UPDLOCK, HOLDLOCK) AS T
...
(
	SELECT TOP (@MaxCount)
		...
		Payload
	FROM
		ScynapseStreamMessage WITH (UPDLOCK)
	WHERE
		ServiceId = @ServiceId
        AND ProviderId = @ProviderId
		AND QueueId = @QueueId
...
(
	SELECT TOP (@Count)
		*
	FROM
		ScynapseStreamMessage AS M WITH (UPDLOCK, HOLDLOCK)
	WHERE
		ServiceId = @ServiceId
        AND ProviderId = @ProviderId
		AND QueueId = @QueueId
...
DELETE FROM ScynapseStreamMessage
OUTPUT
    Deleted.ServiceId,
    Deleted.ProviderId,
    Deleted.QueueId,
...
(
    SELECT TOP (@BatchSize)
        ...
        MessageId
    FROM
        ScynapseStreamDeadLetter WITH (UPDLOCK)
    WHERE
        ServiceId = @ServiceId
        AND ProviderId = @ProviderId
        AND QueueId = @QueueId
