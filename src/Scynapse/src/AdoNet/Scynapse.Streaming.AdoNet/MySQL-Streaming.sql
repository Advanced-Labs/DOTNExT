CREATE TABLE ScynapseStreamMessageSequence
(
    MessageId BIGINT NOT NULL
);
INSERT INTO ScynapseStreamMessageSequence
SELECT 0
WHERE NOT EXISTS (SELECT * FROM ScynapseStreamMessageSequence);
...
UPDATE ScynapseStreamMessageSequence
...
DELETE M
FROM ScynapseStreamMessage AS M
INNER JOIN _Batch AS B
    ON M.ServiceId = B.ServiceId
    AND M.ProviderId = B.ProviderId
    AND M.QueueId = B.QueueId
...
DELETE M
FROM ScynapseStreamDeadLetter AS M
INNER JOIN _Batch AS B
    ON M.ServiceId = B.ServiceId
    AND M.ProviderId = B.ProviderId
    AND M.QueueId = B.QueueId
