...
namespace Scynapse.Streaming.AdoNet;
...
internal class AdoNetQueueAdapterFactory : IQueueAdapterFactory
{
    ...
    /// </summary>
    private ValueTask<RelationalScynapseQueries> GetQueriesAsync()
    {
        // attempt fast path
        return _queries is not null ? new(_queries) : new(CoreAsync());

        // slow path
        async Task<RelationalScynapseQueries> CoreAsync()
        {
            ...
            try
            {
                ...
                return _queries = await RelationalScynapseQueries
    ...
    /// </summary>
    private static readonly ObjectFactory<AdoNetQueueAdapter> AdapterFactory = ActivatorUtilities.CreateFactory<AdoNetQueueAdapter>([typeof(string), typeof(AdoNetStreamOptions), typeof(ClusterOptions), typeof(SimpleQueueCacheOptions), typeof(AdoNetStreamQueueMapper), typeof(RelationalScynapseQueries)]);

    /// <summary>
    /// Factory of <see cref="AdoNetStreamFailureHandler"/> instances.
    /// </summary>
    private static readonly ObjectFactory<AdoNetStreamFailureHandler> HandlerFactory = ActivatorUtilities.CreateFactory<AdoNetStreamFailureHandler>([typeof(bool), typeof(AdoNetStreamOptions), typeof(ClusterOptions), typeof(AdoNetStreamQueueMapper), typeof(RelationalScynapseQueries)]);
}