...
using Scynapse.Runtime.Internal;
        ...
        private async Task DoInitialReadAndUpdateReminders()
        {
            ...
            catch (Exception ex)
            {
                ...
                else
                {
                    ...
                    startedTask.TrySetException(new ScynapseException("ReminderService failed initial load of reminders and cannot guarantee that the service will be eventually start without manual intervention or restarting the silo.", ex));
                }
            }
        }
                ...
                case GrainServiceStatus.Booting:
                    ...
                    else
                    {
                        ...
                        async Task WaitForInitCompletion()
                        {
                            ...
                            {
                                throw new ScynapseException("Reminder Service is still initializing and it is taking a long time. Please retry again later.", ex);
                            }
                            CheckRange();
                        }
                    }
