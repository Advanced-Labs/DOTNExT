...
namespace Scynapse.Runtime.Membership
{
    ...
    /// Every Scynapse deployment has a node   /UniqueDeploymentId
    ...
    public partial class ZooKeeperBasedMembershipTable : IMembershipTable
    {
        ...
        public Task<MembershipTableData> ReadRow(SiloAddress siloAddress)
        {
            return UsingZookeeper(async zk =>
            {
                ...
                {
                    //that's ok because scynapse expects an empty list in case of a missing row
                }
    ...
    /// in scynapse the watcher is only for debugging purposes
    /// </summary>
