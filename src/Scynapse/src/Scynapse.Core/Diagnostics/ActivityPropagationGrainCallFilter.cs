...
namespace Scynapse.Runtime
{
    ...
    internal abstract class ActivityPropagationGrainCallFilter
    {
        ...
        internal const string RpcSystem = "scynapse";
        internal const string ScynapseNamespacePrefix = "Scynapse";
        internal const string ApplicationGrainActivitySourceName = "Genesa.Scynapse.Application";
        internal const string RuntimeActivitySourceName = "Genesa.Scynapse.Runtime";
        ...
        protected static ActivitySource GetActivitySource(IGrainCallContext context) =>
            context.Request.GetInterfaceType().Namespace?.StartsWith(ScynapseNamespacePrefix) == true
                ? RuntimeGrainSource
        ...
        protected static async Task Process(IGrainCallContext context, Activity activity)
        {
            if (activity is not null)
            {
                ...
                {
                    // Custom attributes
                    activity.SetTag("rpc.scynapse.target_id", context.TargetId.ToString());
                    if (context.SourceId is GrainId sourceId)
                    {
                        activity.SetTag("rpc.scynapse.source_id", sourceId.ToString());
                    }
                }
            }
