        ...
        public virtual async Task OnStart(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                    ...
                    .OrderBy(group => group.Key))
                {
                    ...
                    {
                        throw new ScynapseLifecycleCanceledException($"Lifecycle start canceled at stage '{GetStageName(observerGroup.Key)}' by request.");
                    }
            ...
            catch (Exception ex) when (ex is not ScynapseLifecycleCanceledException)
            {
