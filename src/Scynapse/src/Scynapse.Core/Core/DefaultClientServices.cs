...
using Scynapse.Serialization.Internal;
using System;
...
#pragma warning restore 618
            ...
            services.AddSingleton<ITypeNameFilter, AllowScynapseTypes>();
            ...
            services.AddSingleton<IPostConfigureOptions<ScynapseJsonSerializerOptions>, ConfigureScynapseJsonSerializerOptions>();
            services.AddSingleton<ScynapseJsonSerializer>();
        ...
        {
            var services = builder.Services;
            var cfg = builder.Configuration.GetSection("Scynapse");
            ...
            static IProviderBuilder<IClientBuilder> GetRequiredProvider(Dictionary<(string Kind, string Name), Type> knownProviderTypes, string kind, string name)
            {
                ...
                throw new InvalidOperationException($"Could not find {kind} provider named '{name}'. This can indicate that either the 'Genesa.Scynapse.Sdk' package the provider's package are not referenced by your application.");
            }
        ...
        /// <summary>
        /// A <see cref="ITypeNameFilter"/> which allows any type from an assembly containing "Scynapse" in its name to be allowed for the purposes of serialization and deserialization.
        /// </summary>
        private class AllowScynapseTypes : ITypeNameFilter
        {
            /// <inheritdoc />
            public bool? IsTypeNameAllowed(string typeName, string assemblyName)
            {
                if (assemblyName is { Length: > 0} && assemblyName.Contains("Scynapse"))
                {
                    return true;
                }
