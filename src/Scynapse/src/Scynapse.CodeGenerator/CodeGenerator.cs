...
using Scynapse.CodeGenerator.Hashing;
using Scynapse.CodeGenerator.Model;
using Scynapse.CodeGenerator.SyntaxGeneration;
using static Microsoft.CodeAnalysis.CSharp.SyntaxFactory;
using static Scynapse.CodeGenerator.SyntaxGeneration.SymbolExtensions;

namespace Scynapse.CodeGenerator
{
    public class CodeGeneratorOptions
    {
        public List<string> GenerateSerializerAttributes { get; } = new() { "Scynapse.GenerateSerializerAttribute" };
        public List<string> IdAttributes { get; } = new() { "Scynapse.IdAttribute" };
        public List<string> AliasAttributes { get; } = new() { "Scynapse.AliasAttribute" };
        public List<string> ImmutableAttributes { get; } = new() { "Scynapse.ImmutableAttribute" };
        public List<string> ConstructorAttributes { get; } = new() { "Scynapse.ScynapseConstructorAttribute", "Microsoft.Extensions.DependencyInjection.ActivatorUtilitiesConstructorAttribute" };
    ...
    {
        internal const string CodeGeneratorName = "ScynapseCodeGen";
        ...
        public CompilationUnitSyntax GenerateCode(CancellationToken cancellationToken)
        {
            ...
            foreach (var asm in assembliesToExamine)
            {
                foreach (var symbol in asm.GetDeclaredTypes())
                {
                    ...
                    if (FSharpUtilities.IsUnionCase(LibraryTypes, symbol, out var sumType) && ShouldGenerateSerializer(sumType))
                    {
                        ...
                        {
                            throw new ScynapseGeneratorDiagnosticAnalysisException(InaccessibleSerializableTypeDiagnostic.CreateDiagnostic(sumType));
                        }
                    ...
                    else if (ShouldGenerateSerializer(symbol))
                    {
                            ...
                            }))
                        {
                            ...
                            throw new ScynapseGeneratorDiagnosticAnalysisException(ReferenceAssemblyWithGenerateSerializerDiagnostic.CreateDiagnostic(symbol));
                        }

                        if (!Compilation.IsSymbolAccessibleWithin(symbol, Compilation.Assembly))
                        {
                            throw new ScynapseGeneratorDiagnosticAnalysisException(InaccessibleSerializableTypeDiagnostic.CreateDiagnostic(symbol));
                        }
                        ...
                        else
                        {
                            ...
                            {
                                throw new ScynapseGeneratorDiagnosticAnalysisException(CanNotGenerateImplicitFieldIdsDiagnostic.CreateDiagnostic(symbol, fieldIdAssignmentHelper.FailureReason));
                            }
                        ...
                        && (symbol.DeclaredAccessibility == Accessibility.Public || symbol.DeclaredAccessibility == Accessibility.Internal))
                    {
                        ...
                        // Scan grain classes for events (Scynapse Events)
                            ...
                            if (grainEvents.Count > 0)
                            {
                                ...
                                if (eventClassMembers.Length > 0)
                                {
                                    ...
                                    var eventClassDecl = GeneratePartialClassExtensionWithBaseTypes(
                                        symbol,
                                        eventClassMembers,
                                        new[] { "global::Scynapse.ILifecycleParticipant<global::Scynapse.Runtime.IGrainLifecycle>" });
            ...
            var usings = List(new[] { UsingDirective(ParseName("global::Scynapse.Serialization.Codecs")), UsingDirective(ParseName("global::Scynapse.Serialization.GeneratedCodeHelpers")) });
        ...
        private void ComputeAssembliesToExamine(IAssemblySymbol asm, HashSet<IAssemblySymbol> expandedAssemblies)
        {
            ...
            foreach (var attr in attrs)
            {
                ...
                if (declaringAsm is null)
                {
                    ...
                    throw new ScynapseGeneratorDiagnosticAnalysisException(diagnostic);
                }
                else
                {
