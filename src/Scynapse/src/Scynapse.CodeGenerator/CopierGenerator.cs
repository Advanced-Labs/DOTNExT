...
using static Scynapse.CodeGenerator.InvokableGenerator;
using static Scynapse.CodeGenerator.SerializerGenerator;
...
namespace Scynapse.CodeGenerator
{
    internal class CopierGenerator
    {
            ...
            Dictionary<ISerializableTypeDescription, TypeSyntax> defaultCopiers)
        {
            ...
            var baseType = isExceptionType ? QualifiedName(AliasQualifiedName("global", IdentifierName("Scynapse.Serialization.GeneratedCodeHelpers.ScynapseGeneratedCodeHelper")), GenericName(Identifier("ExceptionCopier"), TypeArgumentList(SeparatedList(new[] { type.TypeSyntax, type.BaseType.ToTypeSyntax() }))))
        ...
        private ConstructorDeclarationSyntax GenerateConstructor(string simpleClassName, List<GeneratedFieldDescription> fieldDescriptions, bool isExceptionType)
        {
                ...
                switch (field)
                {
                    ...
                    case CopierFieldDescription or BaseCopierFieldDescription when !field.IsInjected:
                        ...
                        var copier = InvocationExpression(
                            IdentifierName("ScynapseGeneratedCodeHelper").Member(GenericName(Identifier("GetService"), TypeArgumentList(SingletonSeparatedList(field.FieldType)))),
            ...
            static ExpressionSyntax Unwrapped(ExpressionSyntax expr)
            {
                return InvocationExpression(
                    IdentifierName("ScynapseGeneratedCodeHelper").Member("UnwrapService"),
