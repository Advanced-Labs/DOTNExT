...
namespace Scynapse.CodeGenerator
{
    ...
    internal class StatePropertyCodeGenerator
    {
        ...
        public List<StatePropertyDescription> ScanGrainClass(INamedTypeSymbol grainClass)
        {
            ...
            // This prevents generating code for existing Scynapse grains that don't use this feature.
            ...
            IEnumerable<StatePropertyDescription> properties)
        {
            ...
            foreach (var prop in properties)
            {
                ...
                var stateTaskType = GenericName(
                    Identifier("global::Scynapse.StateTask"),
                    TypeArgumentList(SingletonSeparatedList(prop.PropertyType.ToTypeSyntax())));
        ...
        private bool IsGrainKeyInterface(INamedTypeSymbol iface)
        {
            ...
            return name.StartsWith("Scynapse.IGrainWith") && name.EndsWith("Key");
        }
            ...
            string methodId)
        {
            ...
            // Base type: TaskRequest<T> from Scynapse.Runtime namespace
            var baseType = GenericName(
                Identifier("global::Scynapse.Runtime.TaskRequest"),
            ...
            members.Add(MethodDeclaration(PredefinedType(Token(SyntaxKind.VoidKeyword)), "SetTarget")
                ...
                .WithParameterList(ParameterList(SingletonSeparatedList(
                    Parameter(Identifier("holder"))
                        .WithType(ParseTypeName("global::Scynapse.Serialization.Invocation.ITargetHolder")))))
                .WithExpressionBody(ArrowExpressionClause(
            ...
            // GetMethod: public override MethodInfo GetMethod() => ScynapseGeneratedCodeHelper.GetMethodInfoOrDefault(typeof(IFoo), "GetBar", Type.EmptyTypes, Type.EmptyTypes);
            members.Add(MethodDeclaration(ParseTypeName("global::System.Reflection.MethodInfo"), "GetMethod")
                .WithModifiers(TokenList(Token(SyntaxKind.PublicKeyword), Token(SyntaxKind.OverrideKeyword)))
                .WithExpressionBody(ArrowExpressionClause(
                    InvocationExpression(ParseName("ScynapseGeneratedCodeHelper.GetMethodInfoOrDefault"))
                        .WithArgumentList(ArgumentList(SeparatedList(new[] {
            ...
            var classDecl = ClassDeclaration(className)
                ...
                .AddAttributeLists(AttributeList(SingletonSeparatedList(
                    Attribute(ParseName("global::Scynapse.GenerateSerializerAttribute")))))
            ...
            string methodId)
        {
            ...
            //     public override object GetArgument(int index) => index == 0 ? arg0 : ScynapseGeneratedCodeHelper.InvokableThrowArgumentOutOfRange(index, 0);
            //     public override void SetArgument(int index, object value) { if (index == 0) arg0 = (BarType)value; else ScynapseGeneratedCodeHelper.InvokableThrowArgumentOutOfRange(index, 0); }
            // }

            var propertyTypeSyntax = prop.PropertyType.ToTypeSyntax(typeParameterSubstitutions);
            var interfaceTypeSyntax = interfaceType.ToTypeSyntax();

            // Base type: TaskRequest (non-generic) from Scynapse.Runtime namespace
            var baseType = ParseTypeName("global::Scynapse.Runtime.TaskRequest");
            ...
            members.Add(FieldDeclaration(
                ...
                .WithModifiers(TokenList(Token(SyntaxKind.PublicKeyword)))
                .AddAttributeLists(AttributeList(SingletonSeparatedList(
                    Attribute(ParseName("global::Scynapse.IdAttribute"))
            ...
            members.Add(MethodDeclaration(PredefinedType(Token(SyntaxKind.ObjectKeyword)), "GetArgument")
                ...
                .WithExpressionBody(ArrowExpressionClause(
                    ConditionalExpression(
                        ...
                        IdentifierName("arg0"),
                        InvocationExpression(ParseName("ScynapseGeneratedCodeHelper.InvokableThrowArgumentOutOfRange"))
                            .WithArgumentList(ArgumentList(SeparatedList(new[] {
            ...
            // GetMethod: public override MethodInfo GetMethod() => ScynapseGeneratedCodeHelper.GetMethodInfoOrDefault(typeof(IFoo), "SetBar", new Type[] { typeof(T) }, Type.EmptyTypes);
            members.Add(MethodDeclaration(ParseTypeName("global::System.Reflection.MethodInfo"), "GetMethod")
                .WithModifiers(TokenList(Token(SyntaxKind.PublicKeyword), Token(SyntaxKind.OverrideKeyword)))
                .WithExpressionBody(ArrowExpressionClause(
                    InvocationExpression(ParseName("ScynapseGeneratedCodeHelper.GetMethodInfoOrDefault"))
        ...
        private string CreateMethodId(INamedTypeSymbol interfaceType, string methodName, ITypeSymbol[] parameterTypes)
        {
            ...
            var hash = Scynapse.CodeGenerator.Hashing.XxHash32.Hash(System.Text.Encoding.UTF8.GetBytes(methodSignature));
            return Scynapse.CodeGenerator.Hashing.HexConverter.ToString(hash);
        }
        ...
        private AttributeListSyntax GenerateCompoundTypeAliasAttribute(INamedTypeSymbol interfaceType, string methodId)
        {
            ...
            return AttributeList(SingletonSeparatedList(
                Attribute(ParseName("global::Scynapse.CompoundTypeAliasAttribute"))
                            ...
                            ArrayCreationExpression(
                                ...
                                .WithInitializer(InitializerExpression(SyntaxKind.ArrayInitializerExpression,
                                    SeparatedList<ExpressionSyntax>(new ExpressionSyntax[] {
                                        ...
                                        TypeOfExpression(ParseTypeName("global::Scynapse.Runtime.GrainReference")),
