...
namespace Scynapse.Clustering.Cassandra.Hosting;
...
{
    /// <summary>
    /// Configures Scynapse clustering using Cassandra.
    /// </summary>
    ...
    /// <param name="keyspace">The Cassandra keyspace, which defaults to <c>scynapse</c>.</param>
    /// <returns>The client builder.</returns>
    public static IClientBuilder UseCassandraClustering(this IClientBuilder builder, string connectionString, string keyspace = "scynapse") =>
        builder.ConfigureServices(services =>
        {
    ...
    /// <param name="keyspace">The Cassandra keyspace, which defaults to <c>scynapse</c>.</param>
    /// <returns>The silo builder.</returns>
    public static ISiloBuilder UseCassandraClustering(this ISiloBuilder builder, string connectionString, string keyspace = "scynapse") =>
        builder.ConfigureServices(services =>
        {
            services.Configure<CassandraClusteringOptions>(o => o.ConfigureClient(connectionString, keyspace));
            services.AddSingleton<IMembershipTable, CassandraClusteringTable>();
