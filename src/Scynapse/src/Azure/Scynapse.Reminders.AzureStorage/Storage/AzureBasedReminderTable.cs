...
using Scynapse.AzureUtils.Utilities;
        ...
        private ReminderEntry ConvertFromTableEntry(ReminderTableEntry tableEntry, string eTag)
        {
            ...
            finally
            {
                ...
                if (!tableEntry.ServiceId.Equals(serviceIdStr))
                {
                    ...
                    throw new ScynapseException($"Read a reminder entry for wrong Service id. Read {tableEntry}, but my service id is {serviceIdStr}. Going to discard it.");
                }
            }
        }
