...
namespace Scynapse.Configuration
{
    ...
    public class EventHubOptionsValidator : IConfigurationValidator
    {
        ...
        public void ValidateConfiguration()
        {
            ...
            {
                throw new ScynapseConfigurationException($"Azure Event Hub connection not configured for stream provider options {nameof(EventHubOptions)} with name \"{name}\". Use the {options.GetType().Name}.{nameof(EventHubOptions.ConfigureEventHubConnection)} method to configure the connection.");
            }

            if (string.IsNullOrEmpty(options.ConsumerGroup))
            {
                throw new ScynapseConfigurationException($"{nameof(EventHubOptions)} on stream provider {this.name} is invalid. {nameof(EventHubOptions.ConsumerGroup)} is invalid");
            }

            if (string.IsNullOrEmpty(options.EventHubName))
            {
                throw new ScynapseConfigurationException($"{nameof(EventHubOptions)} on stream provider {this.name} is invalid. {nameof(EventHubOptions.EventHubName)} is invalid");
            }
        }
    }
    ...
    public class StreamCheckpointerConfigurationValidator : IConfigurationValidator
    {
        ...
        public void ValidateConfiguration()
        {
            ...
            if (checkpointerFactory == null)
                throw new ScynapseConfigurationException($"No IStreamQueueCheckpointer is configured with PersistentStreamProvider {this.name}. Please configure one.");
        }
    }
