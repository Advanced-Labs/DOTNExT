...
namespace Scynapse.DurableJobs.AzureStorage;
...
public sealed partial class AzureStorageJobShardManager : JobShardManager
{
    ...
    public override async Task<List<Scynapse.DurableJobs.IJobShard>> AssignJobShardsAsync(DateTimeOffset maxShardStartTime, CancellationToken cancellationToken)
    {
        await InitializeIfNeeded(cancellationToken);
        LogAssigningShards(_logger, SiloAddress, maxShardStartTime, _containerName);

        var result = new List<Scynapse.DurableJobs.IJobShard>();
    ...
    public override async Task<Scynapse.DurableJobs.IJobShard> CreateShardAsync(DateTimeOffset minDueTime, DateTimeOffset maxDueTime, IDictionary<string, string> metadata, CancellationToken cancellationToken)
    {
        await InitializeIfNeeded(cancellationToken);
    ...
    public override async Task UnregisterShardAsync(Scynapse.DurableJobs.IJobShard shard, CancellationToken cancellationToken)
    {
