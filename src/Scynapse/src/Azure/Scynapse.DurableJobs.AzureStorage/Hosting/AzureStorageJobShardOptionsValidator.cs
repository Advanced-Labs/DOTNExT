...
public class AzureStorageJobShardOptionsValidator : IConfigurationValidator
{
    ...
    public void ValidateConfiguration()
    {
        ...
        {
            throw new ScynapseConfigurationException($"Invalid configuration for {nameof(AzureStorageJobShardOptions)} with name '{_name}'. {nameof(_options.BlobServiceClient)} is required.");
        }

        if (string.IsNullOrWhiteSpace(_options.ContainerName))
        {
            throw new ScynapseConfigurationException($"Invalid configuration for {nameof(AzureStorageJobShardOptions)} with name '{_name}'. {nameof(_options.ContainerName)} is required.");
        }
    }
}
