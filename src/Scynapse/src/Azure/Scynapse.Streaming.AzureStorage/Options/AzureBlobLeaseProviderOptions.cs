...
using Scynapse.Streaming.AzureStorage;
...
namespace Scynapse.Configuration
{
    ...
    public class AzureBlobLeaseProviderOptionsValidator : IConfigurationValidator
    {
        ...
        public void ValidateConfiguration()
        {
            ...
            {
                throw new ScynapseConfigurationException($"Named option {nameof(AzureBlobLeaseProviderOptions)} of name {this.name} is invalid.  Name cannot be empty or whitespace.");
            }

            if (this.options.CreateClient is null)
            {
                throw new ScynapseConfigurationException($"No credentials specified for Azure Blob Service lease provider \"{name}\". Use the {options.GetType().Name}.{nameof(AzureBlobLeaseProviderOptions.ConfigureBlobServiceClient)} method to configure the Azure Blob Service client.");
            }
            ...
            catch (ArgumentException e)
            {
                ...
                throw new ScynapseConfigurationException(errorStr , e);
            }
        }
    }
