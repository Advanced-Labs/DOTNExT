...
using static Scynapse.Persistence.Cosmos.CosmosIdSanitizer;
        ...
        for (var retry = 0; retry <= maxRetries; ++retry)
        {
            ...
            if (containerResponse.StatusCode == HttpStatusCode.OK || containerResponse.StatusCode == HttpStatusCode.Created)
            {
                ...
                if (_partitionKeyPath == GRAINTYPE_PARTITION_KEY_PATH &&
                    ...
                    throw new ScynapseConfigurationException("Custom partition key provider is not compatible with partition key path set to /GrainType");
            }
