using System.Net;
using Scynapse.Clustering.Cosmos.Models;
...
internal partial class CosmosMembershipTable : IMembershipTable
{
    ...
    public async Task UpdateIAmAlive(MembershipEntry entry)
    {
        ...
        if (_self is not { } selfRow)
        {
            ...
            if (response.StatusCode != HttpStatusCode.OK)
            {
                ...
                throw new ScynapseException($"Unable to query for SiloEntity {entry.ToFullString()}");
            }
    ...
    private static MembershipEntry ParseEntity(SiloEntity entity)
    {
        ...
        {
            throw new ScynapseException($"SuspectingSilos.Length of {suspectingSilos.Count} as read from Azure Cosmos DB is not equal to SuspectingTimes.Length of {suspectingTimes.Count}");
        }
