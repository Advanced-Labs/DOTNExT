...
using Scynapse.TestingHost.InMemoryTransport;
using Scynapse.TestingHost.UnixSocketTransport;
using System.Net;
...
{
    /// <summary>
    /// A host class for local testing with Scynapse using in-process silos. 
    ...
    public class TestCluster : IDisposable, IAsyncDisposable
    {
        ...
        public async Task DeployAsync()
        {
            ...
            catch (Exception ex)
            {
                ...
                // Due to the way MS tests works, if the original exception is an Scynapse exception,
        ...
        public async Task InitializeClientAsync()
        {
            ...
            if (options.UseTestClusterMembership)
            {
                ...
                {
                    clustering[$"Scynapse:Clustering:Gateways:{i++}"] = v.ToString();
                }

                clustering["Scynapse:Clustering:ProviderType"] = "Development";
            ...
            this.ClientHost = TestClusterHostFactory.CreateClusterClient(
                ...
                hostBuilder =>
                {
                    hostBuilder.UseScynapseClient((context, clientBuilder) =>
                    {
        ...
        public async Task<SiloHandle> DefaultCreateSiloAsync(string siloName, IConfiguration configuration)
        {
            ...
            {
                hostBuilder.UseScynapse((context, siloBuilder) =>
                {
                    Enum.TryParse<ConnectionTransportType>(context.Configuration[nameof(TestClusterOptions.ConnectionTransport)], out var transport);
                    switch (transport)
                    {
