    ...
    ReactDom.render(
      <span id="version" style={{ marginLeft: 40 }}>
        ...
        <a
          ...
          href="https://github.com/ScynapseContrib/ScynapseDashboard/"
        >
          Source
        </a>
      </span>,
...
function filterByDashboard(x: GrainStatItem): boolean {
  if (x.grainType == undefined) {
    const dashboardGrain = x.grain!.startsWith('ScynapseDashboard.');
    return !dashboardGrain;
  } else {
    const dashboardGrain = x.grainType.startsWith('ScynapseDashboard.');
    return !dashboardGrain;
  }
}
...
function filterBySystem(x: GrainStatItem): boolean {
  if (x.grainType == undefined) {
    const systemGrain = x.grain!.startsWith('Scynapse.');
    return !systemGrain;
  } else {
    const systemGrain = x.grainType.startsWith('Scynapse.');
    return !systemGrain;
  }
}
