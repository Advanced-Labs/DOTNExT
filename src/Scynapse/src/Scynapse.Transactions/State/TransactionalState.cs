...
using Scynapse.Transactions.State;
...
{
    /// <summary>
    /// Stateful facet that respects Scynapse transaction semantics
    /// </summary>
        ...
        where TState : class, new()
    {
        ...
        public Task<TResult> PerformRead<TResult>(Func<TState, TResult> operation)
        {
            ...
            return this.queue.RWLock.EnterLock<TResult>(info.TransactionId, info.Priority, recordedaccesses, true,
                () =>
                {
                    ...
                    {
                        throw new ScynapseCascadingAbortException(info.TransactionId.ToString());
                    }
        ...
        public Task<TResult> PerformUpdate<TResult>(Func<TState, TResult> updateAction)
        {
            ...
            if (info.IsReadOnly)
            {
                throw new ScynapseReadOnlyViolatedException(info.Id);
            }
