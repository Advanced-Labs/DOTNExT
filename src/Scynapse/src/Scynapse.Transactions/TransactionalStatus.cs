...
namespace Scynapse.Transactions
{
    ...
    public static class TransactionalStatusExtensions
    {
        ...
        public static ScynapseTransactionException ConvertToUserException(this TransactionalStatus status, string transactionId, Exception exception)
        {
            switch (status)
            {
                case TransactionalStatus.PrepareTimeout:
                    return new ScynapseTransactionPrepareTimeoutException(transactionId, exception);

                case TransactionalStatus.CascadingAbort:
                    return new ScynapseCascadingAbortException(transactionId, exception);

                case TransactionalStatus.BrokenLock:
                    return new ScynapseBrokenTransactionLockException(transactionId, "before prepare", exception);

                case TransactionalStatus.LockValidationFailed:
                    return new ScynapseBrokenTransactionLockException(transactionId, "when validating accesses during prepare", exception);

                case TransactionalStatus.ParticipantResponseTimeout:
                    return new ScynapseTransactionTransientFailureException(transactionId, $"transaction agent timed out waiting for read-only transaction participant responses ({status})", exception);

                case TransactionalStatus.TMResponseTimeout:
                    return new ScynapseTransactionInDoubtException(transactionId, $"transaction agent timed out waiting for read-only transaction participant responses ({status})", exception);

                case TransactionalStatus.CommitFailure:
                    return new ScynapseTransactionAbortedException(transactionId, $"Unable to commit transaction ({status})", exception);

                default:
                    return new ScynapseTransactionInDoubtException(transactionId, $"failure during transaction commit, status={status}", exception);
            }
        }
    }
}
