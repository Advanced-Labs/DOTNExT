...
namespace Scynapse.Serialization.Internal
{
    public static class ReferencedAssemblyProvider
    {
        ...
        public static void AddFromDependencyContext(HashSet<Assembly> parts, Assembly assembly = null)
        {
            ...
            {
                if (!lib.Name.Contains("Scynapse.Serialization", StringComparison.Ordinal) && !lib.Dependencies.Any(dep => dep.Name.Contains("Scynapse.Serialization", StringComparison.Ordinal)))
                {
                    continue;
                }
