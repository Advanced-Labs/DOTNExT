...
namespace Scynapse.GrainDirectory.Redis
{
    public partial class RedisGrainDirectory : IGrainDirectory, ILifecycleParticipant<ISiloLifecycle>
    {
        ...
        public async Task<GrainAddress?> Lookup(GrainId grainId)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (IsRedisException(ex))
                    throw new ScynapseException($"Lookup failed for {grainId} : {ex}");
                else
                    throw;
            }
        }
        ...
        public async Task<GrainAddress?> Register(GrainAddress address, GrainAddress? previousAddress)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (IsRedisException(ex))
                {
                    throw new ScynapseException($"Register failed for {address.GrainId} ({value}) : {ex}");
                }
                else
                {
                    throw;
        ...
        public async Task Unregister(GrainAddress address)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (IsRedisException(ex))
                    throw new ScynapseException($"Unregister failed for {address.GrainId} ({JsonSerializer.Serialize(address)}) : {ex}");
                else
                    throw;
            }
        }
