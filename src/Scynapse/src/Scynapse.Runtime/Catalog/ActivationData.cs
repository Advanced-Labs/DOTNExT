...
/// <summary>
/// Maintains additional per-activation state that is required for Scynapse internal operations.
    ...
    private async Task ActivateAsync(Dictionary<string, object>? requestContextData, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            catch (Exception exception)
            {
                ...
                var sourceException = (exception as ScynapseLifecycleCanceledException)?.InnerException ?? exception;
