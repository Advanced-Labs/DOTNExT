...
/*
The grain directory in Scynapse is a key-value store where the key is a grain identifier and the value is a registration entry which points to an active silo which (potentially)
hosts the grain.
...
internal sealed partial class DistributedGrainDirectory : SystemTarget, IGrainDirectory, IGrainDirectoryClient, ILifecycleParticipant<ISiloLifecycle>, DistributedGrainDirectory.ITestHooks
{
        ...
        [CallerMemberName] string operation = "")
    {
        ...
        while (true)
        {
            ...
            catch (ScynapseMessageRejectionException) when (attempts < MaxAttempts && !cancellationToken.IsCancellationRequested)
            {
