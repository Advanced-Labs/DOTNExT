...
#nullable enable
namespace Scynapse.Runtime.GrainDirectory
{
    internal sealed partial class LocalGrainDirectory : ILocalGrainDirectory, ISiloStatusListener, ILifecycleParticipant<ISiloLifecycle>
    {
        ...
        public SiloAddress? CheckIfShouldForward(GrainId grainId, int hopCount, string operationDescription)
        {
            ...
            if (hopCount >= HOP_LIMIT)
            {
                ...
                throw new ScynapseException($"Silo {MyAddress} is not owner of {grainId}, cannot forward {operationDescription} to owner {owner} because hop limit is reached");
            }
        ...
        public async Task<AddressAndTag> LookupAsync(GrainId grainId, int hopCount = 0)
        {
            ...
            else
            {
                ...
                {
                    throw new ScynapseException($"Current directory at {MyAddress} is not stable to perform the lookup for grainId {grainId} (it maps to {forwardAddress}, which is not a valid silo). Retry later.");
                }
