    ...
    internal partial class MembershipTableManager : IHealthCheckParticipant, ILifecycleParticipant<ISiloLifecycle>, IDisposable
    {
        ...
        private async Task Start()
        {
            try
            {
                ...
                if (!refreshed)
                {
                    throw new ScynapseException("Failed to perform initial membership refresh and cleanup.");
                }
        ...
        public async Task UpdateStatus(SiloStatus status)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    throw new ScynapseException($"Silo {myAddress} failed to update its status to {status} in the membership table due to write contention on the table after {numCalls} attempts.");
                }
            }
            catch (Exception exc)  when (!wasThrownLocally)
            {
                ...
                throw new ScynapseException($"Silo {myAddress} failed to update its status to {status} in the table due to failures (socket failures or table read/write failures) after {numCalls} attempts", exc);
            }
        }
