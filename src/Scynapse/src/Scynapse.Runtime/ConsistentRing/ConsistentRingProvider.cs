...
namespace Scynapse.Runtime.ConsistentRing
{
    /// <summary>
        ...
        IConsistentRingProvider, ISiloStatusListener, IDisposable
    {
        ...
        internal void AddServer(SiloAddress silo)
        {
            lock (membershipRingList)
            {
                ...
                if (!(membershipRingList.Count == 0 || myOldIndex != -1))
                    throw new ScynapseException(string.Format("{0}: Couldn't find my position in the ring {1}.", MyAddress, Utils.EnumerableToString(membershipRingList)));
        ...
        internal void RemoveServer(SiloAddress silo)
        {
            lock (membershipRingList)
            {
                ...
                if (myNewIndex == -1)
                    throw new ScynapseException($"{MyAddress}: Couldn't find my position in the ring {this}.");
        ...
        private SiloAddress CalculateTargetSilo(uint hash, bool excludeThisSiloIfStopping = true)
        {
            ...
            lock (membershipRingList)
            {
                ...
                // if you want to stick to counter-clockwise, change the responsibility definition in 'In()' method & responsibility defs in ScynapseReminderMemory
                ...
                if (siloAddress == null)
                {
                    ...
                    // if you go back to their counter-clockwise policy, then change the 'In()' method in ScynapseReminderMemory
