...
internal sealed class PluginAssemblyLoader
{
        ...
        try
        {
            ...
            try
            {
                ...
                pluginLoader = PluginLoader.CreateFromAssemblyFile(
                    ...
                    config =>
                    {
                        // Share Scynapse types between host and plugin to avoid type identity issues
                        config.PreferSharedTypes = true;

                        // Enable unloading support - critical for runtime grain replacement
                        config.IsUnloadable = true;

                        // Configure explicit shared types from Scynapse runtime
    ...
    /// This method discovers all Scynapse-relevant assemblies in the same AssemblyLoadContext,
    ...
    private Type[] GetOrCreateSharedTypes()
    {
        ...
        _cachedSharedTypes = GetScynapseSharedTypes();
        return _cachedSharedTypes;
    }

    /// <summary>
    /// Returns the comprehensive list of Scynapse types that should be shared across plugin boundaries.
    /// Uses reflection to automatically discover all Scynapse types from loaded assemblies,
    /// eliminating the need for manual maintenance and avoiding circular dependency issues.
    /// </summary>
    private Type[] GetScynapseSharedTypes()
    {
        var sharedTypes = new List<Type>();

        // Scan all currently loaded assemblies that belong to Scynapse
        var scynapseAssemblies = AppDomain.CurrentDomain.GetAssemblies()
            .Where(a =>
            {
                var name = a.GetName().Name;
                return name != null &&
                       (name.StartsWith("Scynapse", StringComparison.Ordinal) ||
                        name.StartsWith("Genesa.Scynapse", StringComparison.Ordinal));
            })
            .ToList();

        _logger.LogDebug(
            "Scanning {AssemblyCount} Scynapse assemblies for shared types",
            scynapseAssemblies.Count);

        foreach (var assembly in scynapseAssemblies)
        {
            try
            {
                // Get all exported (public) types from Scynapse namespaces
                var types = assembly.GetExportedTypes()
                    .Where(t => t.Namespace?.StartsWith("Scynapse", StringComparison.Ordinal) == true)
                    .Where(t =>
