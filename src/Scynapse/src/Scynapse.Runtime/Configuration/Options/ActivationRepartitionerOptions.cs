...
internal sealed class ActivationRepartitionerOptionsValidator(IOptions<ActivationRepartitionerOptions> options) : IConfigurationValidator
{
    ...
    public void ValidateConfiguration()
    {
        ...
        {
            throw new ScynapseConfigurationException($"{nameof(ActivationRepartitionerOptions.ProbabilisticFilteringMaxAllowedErrorRate)} must be inclusive between [0.001 - 0.01](0.1% - 1%)");
        }
    }

    private static void ThrowMustBeGreaterThanOrEqualToZero(string propertyName)
        => throw new ScynapseConfigurationException($"{propertyName} must be greater than or equal to 0.");

    private static void ThrowMustBeGreaterThanZero(string propertyName)
        => throw new ScynapseConfigurationException($"{propertyName} must be greater than 0.");

    private static void ThrowMustBeGreaterThanOrEqualTo(string name1, string name2)
        => throw new ScynapseConfigurationException($"{name1} must be greater than or equal to {name2}.");
}