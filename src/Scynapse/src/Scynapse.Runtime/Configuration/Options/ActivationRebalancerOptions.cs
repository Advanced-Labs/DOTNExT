    ...
    IOptions<DeploymentLoadPublisherOptions> publisherOptions) : IConfigurationValidator
{
    ...
    public void ValidateConfiguration()
    {
        ...
        {
            throw new ScynapseConfigurationException($"{nameof(ActivationRebalancerOptions.MaxStagnantCycles)} must be greater than 0");
        }

        if (_options.EntropyQuantum <= 0d || _options.EntropyQuantum > 0.1d)
        {
            throw new ScynapseConfigurationException($"{nameof(ActivationRebalancerOptions.EntropyQuantum)} must be in greater than 0, and less or equal 0.1");
        }

        if (_options.AllowedEntropyDeviation <= 0d || _options.AllowedEntropyDeviation > 0.1d)
        {
            throw new ScynapseConfigurationException($"{nameof(ActivationRebalancerOptions.AllowedEntropyDeviation)} must be in greater than 0, and less or equal 0.1");
        }

        if (_options.CycleNumberWeight <= 0d || _options.CycleNumberWeight > 1d)
        {
            throw new ScynapseConfigurationException($"{nameof(ActivationRebalancerOptions.CycleNumberWeight)} must be in greater than 0, and less or equal to 1");
        }

        if (_options.SiloNumberWeight < 0d || _options.SiloNumberWeight > 1d)
        {
            throw new ScynapseConfigurationException($"{nameof(ActivationRebalancerOptions.SiloNumberWeight)} must be in greater than or equal to 0, and less or equal to 1");
        }

        if (_options.ActivationMigrationCountLimit < 1)
        {
            throw new ScynapseConfigurationException($"{nameof(ActivationRebalancerOptions.ActivationMigrationCountLimit)} must be greater than 0");
        }

        if (_options.ScaledEntropyDeviationActivationThreshold < 1_000)
        {
            throw new ScynapseConfigurationException($"{nameof(ActivationRebalancerOptions.ScaledEntropyDeviationActivationThreshold)} must be greater than or equal to 1000");
        }
    }
}