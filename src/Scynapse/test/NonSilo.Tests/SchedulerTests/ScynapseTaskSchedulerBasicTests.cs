...
{
    /// <summary>
    /// Test implementation of IGrainContext for unit testing the Scynapse task scheduler without requiring a full grain activation.
    /// </summary>
    ...
    /// <summary>
    /// Basic tests for the Scynapse task scheduler, which is the core component responsible for scheduling grain work items.
    /// The scheduler ensures single-threaded execution semantics for grains while efficiently utilizing thread pool resources.
    /// These tests verify fundamental scheduling operations without requiring a full Scynapse runtime.
    /// </summary>
    [TestCategory("BVT"), TestCategory("Scheduler")]
    public class ScynapseTaskSchedulerBasicTests : IDisposable
    {
        private readonly ITestOutputHelper output;
        private static readonly object Lockable = new object();
        private readonly UnitTestSchedulingContext rootContext;
        private readonly ILoggerFactory loggerFactory;
        public ScynapseTaskSchedulerBasicTests(ITestOutputHelper output)
        {
            this.output = output;
        ...
        /// <summary>
        /// Tests that tasks can be scheduled and executed on the Scynapse activation task scheduler.
        ...
        private void LogContext(string what)
        {
            lock (Lockable)
            {
                this.output.WriteLine(
                    ...
                    + " Scynapse-RuntimeContext.Current={4}",
                    what,
