...
namespace Scynapse.CodeGenerator.Tests;

/// <summary>
/// Tests for the Scynapse source generator that generates serialization and RPC code.
///
/// The Scynapse source generator uses Roslyn source generators to:
...
/// - Generate metadata for Scynapse runtime
///
/// Key features tested:
...
/// </summary>
public class ScynapseSourceGeneratorTests
{
    /// <summary>
...
public class InterfaceCtorParam
{
    ...
    [Fact]
    public Task TestClassesWithScynapseConstructorAnnotation() => AssertSuccessfulSourceGeneration(
@"using Scynapse;

namespace TestProject;

public class ClassWithScynapseConstructor
{
    [Id(0)]
    public int Value { get; set; }
    ...
    [ScynapseConstructor]
    public ClassWithScynapseConstructor(int value, string name)
    {
        Value = value;
        Name = name;
    }

    public ClassWithScynapseConstructor() { }
}");
...
public class ClassWithImplicitFieldIds
{
    ...
    /// - Proper integration with Scynapse runtime
    /// </summary>
    [Fact]
...
public class BasicGrain : Grain, IBasicGrain
{
    ...
    /// Scynapse supports multiple grain key types:
    /// - Integer keys
    /// - GUID keys
    /// - String keys
...
@"using Scynapse;
using Scynapse.Serialization.Activators;
...
public class DemoClass
{
    ...
    public async Task EmitsWarningForGenerateSerializerInReferenceAssembly()
    {
        ...
        // This triggers the Scynapse code generator's logic to emit a diagnostic if [GenerateSerializer] is used in such an assembly.
        ...
        var generator = new ScynapseSerializationSourceGenerator();
    ...
    /// <summary>
    /// Helper method that runs the Scynapse source generator on the provided code
    ...
    private static async Task AssertSuccessfulSourceGeneration(string code)
    {
        ...
        Assert.Equal($"{projectName}.scynapse.g.cs", result.GeneratedSources[0].HintName);
    ...
    /// <summary>
    /// Creates a Roslyn compilation with the necessary Scynapse references.
    /// This simulates the build environment where the source generator runs,
    /// including all required Scynapse assemblies and .NET framework references.
    /// </summary>
    private static async Task<CSharpCompilation> CreateCompilation(string sourceCode, string assemblyName = "TestProject")
    {
        ...
        // Add the Scynapse Scynapse.Core.Abstractions assembly
        references = references.AddRange(
            // Scynapse.Core.Abstractions
